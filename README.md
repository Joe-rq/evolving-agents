...
demo/         # demo 图（01-05 对照结果 + 06 符号回归进化 + 07 正则合成进化）
img/          # 图
```
...
在**四个领域**验证过：量化因子挖掘（私有，真实案例）+ 2048（公开，带失忆组对照）+ 符号回归（公开，可观察的生成器进化）+ 正则合成（公开，程序合成任务的进化）。2048 对照结果 null（p=0.079）——机制可观察地工作，但有效性增益未被统计证明；符号回归 + 正则合成展示了生成器从成功嫁接出新候选的可观察进化（跨公式发现与程序合成两种任务类型），但都是种子化轨迹、非统计证明。跨领域泛化仍是 roadmap，不是 claim。→ [诚实声明边界](content/honest-boundary.md)
